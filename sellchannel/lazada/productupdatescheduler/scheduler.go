// Package productupdatescheduler periodically pushes pending product edits to
// Lazada via the shared productupdatesync.Syncer.
package productupdatescheduler
...
type Scheduler struct {
	cron   *cron.Cron
	syncer *productupdatesync.Syncer
...
func New(syncer *productupdatesync.Syncer, tokens *tokenstore.Store) *Scheduler {
	return &Scheduler{
		cron:   cron.New(),
		syncer: syncer,
		tokens: tokens,
		spec:   getEnv("LAZADA_PRODUCT_UPDATE_SPEC", "@every 5m"),
		limit:  getEnvInt("LAZADA_PRODUCT_UPDATE_LIMIT", 20),
	}
}
...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	s.cron.Start()
	logger.Info("lazada product-update scheduler started", "spec", s.spec, "limit", s.limit)

	<-ctx.Done()
	s.cron.Stop()
	logger.Info("lazada product-update scheduler stopped")
	return nil
}

func (s *Scheduler) run(ctx context.Context) {
	if err := s.syncer.UpdatePending(ctx, s.limit); err != nil {
		logger.Error("lazada product update failed", "error", err)
	}
}
