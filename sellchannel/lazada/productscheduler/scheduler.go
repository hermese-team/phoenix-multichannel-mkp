// Package productscheduler periodically pushes pending product price/stock to
// Lazada via the shared productsync.Syncer.
package productscheduler
...
func New(syncer *productsync.Syncer, tokens *tokenstore.Store) *Scheduler {
	return &Scheduler{
		cron:   cron.New(),
		syncer: syncer,
		tokens: tokens,
		spec:   getEnv("LAZADA_PRODUCT_SYNC_SPEC", "@every 5m"),
		limit:  getEnvInt("LAZADA_PRODUCT_SYNC_LIMIT", 50),
	}
}
...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	s.cron.Start()
	logger.Info("lazada product scheduler started", "spec", s.spec, "limit", s.limit)

	<-ctx.Done()
	s.cron.Stop()
	logger.Info("lazada product scheduler stopped")
	return nil
}

func (s *Scheduler) run(ctx context.Context) {
	if err := s.syncer.SyncPending(ctx, s.limit); err != nil {
		logger.Error("lazada product sync failed", "error", err)
	}
}
