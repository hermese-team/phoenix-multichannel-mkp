// Package imagescheduler periodically uploads + attaches pending product images
// on Lazada via the shared imagesync.Syncer.
package imagescheduler
...
func New(syncer *imagesync.Syncer, tokens *tokenstore.Store) *Scheduler {
	return &Scheduler{
		cron:   cron.New(),
		syncer: syncer,
		tokens: tokens,
		spec:   getEnv("LAZADA_IMAGE_SPEC", "@every 5m"),
		limit:  getEnvInt("LAZADA_IMAGE_LIMIT", 20),
	}
}
...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	s.cron.Start()
	logger.Info("lazada image scheduler started", "spec", s.spec, "limit", s.limit)

	<-ctx.Done()
	s.cron.Stop()
	logger.Info("lazada image scheduler stopped")
	return nil
}

func (s *Scheduler) run(ctx context.Context) {
	if err := s.syncer.SyncPending(ctx, s.limit); err != nil {
		logger.Error("lazada image sync failed", "error", err)
	}
}
