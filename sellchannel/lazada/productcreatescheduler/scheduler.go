// Package productcreatescheduler periodically creates pending products on Lazada
// via the shared productcreatesync.Syncer.
package productcreatescheduler
...
type Scheduler struct {
	cron   *cron.Cron
	syncer *productcreatesync.Syncer
...
func New(syncer *productcreatesync.Syncer, tokens *tokenstore.Store) *Scheduler {
	return &Scheduler{
		cron:   cron.New(),
		syncer: syncer,
		tokens: tokens,
		spec:   getEnv("LAZADA_PRODUCT_CREATE_SPEC", "@every 5m"),
		limit:  getEnvInt("LAZADA_PRODUCT_CREATE_LIMIT", 20),
	}
}
...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	s.cron.Start()
	logger.Info("lazada product-create scheduler started", "spec", s.spec, "limit", s.limit)

	<-ctx.Done()
	s.cron.Stop()
	logger.Info("lazada product-create scheduler stopped")
	return nil
}

func (s *Scheduler) run(ctx context.Context) {
	if err := s.syncer.CreatePending(ctx, s.limit); err != nil {
		logger.Error("lazada product create failed", "error", err)
	}
}
