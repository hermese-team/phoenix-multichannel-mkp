...
func main() {
	...
	sched, err := lazada.NewProductOffsaleScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada offsale scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("offsale scheduler error: %v", err)
	}
}
