...
func main() {
	...
	sched, err := lazada.NewProductCreateScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada product-create scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("product-create scheduler error: %v", err)
	}
}
