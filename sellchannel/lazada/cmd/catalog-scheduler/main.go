...
func main() {
	...
	sched, err := lazada.NewCatalogScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada catalog scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("catalog scheduler error: %v", err)
	}
}
