...
func main() {
	...
	sched, err := lazada.NewSellableStockScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada sellable-stock scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("sellable-stock scheduler error: %v", err)
	}
}
