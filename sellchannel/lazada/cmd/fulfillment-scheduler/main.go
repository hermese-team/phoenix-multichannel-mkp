...
func main() {
	...
	sched, err := lazada.NewFulfillmentScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada fulfillment scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("fulfillment scheduler error: %v", err)
	}
}
