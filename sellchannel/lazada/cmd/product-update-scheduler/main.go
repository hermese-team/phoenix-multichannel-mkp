...
func main() {
	...
	sched, err := lazada.NewProductUpdateScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada product-update scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("product-update scheduler error: %v", err)
	}
}
