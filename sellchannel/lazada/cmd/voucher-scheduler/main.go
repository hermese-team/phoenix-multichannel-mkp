...
func main() {
	...
	sched, err := lazada.NewVoucherScheduler(cfg.Lazada, cfg.Postgres, cfg.Redis)
	if err != nil {
		log.Fatalf("init lazada voucher scheduler: %v", err)
	}
	...
	if err := sched.Start(ctx); err != nil {
		log.Fatalf("voucher scheduler error: %v", err)
	}
}
