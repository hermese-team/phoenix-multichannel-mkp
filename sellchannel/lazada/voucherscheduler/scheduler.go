// Package voucherscheduler periodically creates pending seller vouchers on
// Lazada via the shared vouchersync.Syncer.
package voucherscheduler
...
func New(syncer *vouchersync.Syncer, tokens *tokenstore.Store) *Scheduler {
	return &Scheduler{
		cron:   cron.New(),
		syncer: syncer,
		tokens: tokens,
		spec:   getEnv("LAZADA_VOUCHER_SPEC", "@every 5m"),
		limit:  getEnvInt("LAZADA_VOUCHER_LIMIT", 20),
	}
}
...
func (s *Scheduler) Start(ctx context.Context) error {
	...
	s.cron.Start()
	logger.Info("lazada voucher scheduler started", "spec", s.spec, "limit", s.limit)

	<-ctx.Done()
	s.cron.Stop()
	logger.Info("lazada voucher scheduler stopped")
	return nil
}

func (s *Scheduler) run(ctx context.Context) {
	if err := s.syncer.CreatePending(ctx, s.limit); err != nil {
		logger.Error("lazada voucher create failed", "error", err)
	}
}
