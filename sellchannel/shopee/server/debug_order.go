...
// then publishes a raw event to raw.accepted.shopee.v1.dev for the consumer to enrich.
func (s *Server) debugFetchOrder(c *gin.Context) {
	...
	// 2. publish raw event to raw.accepted.shopee.v1.dev — consumer will fetch full detail
	if s.intake != nil {
		evt := intake.RawEvent{
			ShopID:    req.ShopID,
			OrderSN:   req.OrderSN,
